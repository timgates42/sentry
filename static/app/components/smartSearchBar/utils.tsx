...
/**
 * Special logic for tags of type DATE:
 *
 * 1. If at the beginning of the token value (cursor === node.location.start.offset),
 *    show the suggested values (-1h, on date, after date, etc)
 * 2. If a valid operator exists and we are within the token value, display a date picker
 */
export const getDateTagAutocompleteGroups = ({
  tagName,
}: {
  cursor: number;
  tagName: string;
  tokenCursorOffset: number;
}): AutocompleteGroup[] => {
  return [
    {
